...
df_data = DataFrame(quick_open("true_value_redo.happy")["prediction"])[:,3:1002]
...
lines!(ax, 0:0.1:49.9, quick_open("true_value_redo.happy")["noise_free"][1:500], color="grey50", label = "True amplitude", linewidth=2)
...
scatter([1:100], data[1:100,4], color="grey90", label="OU + thermal", linewidth=2)
plot!(data[1:100,3], linecolor="black", label="OU", linewidth=1.2, linestyle=:dash)
plot!(data[1:100,1],ribbon = data[1:100,2], linecolor="grey40", fillcolor="grey50", label="EM prediction", linewidth=2)
...
f = Figure()
ax = Axis(f[1, 1],
    xlabel = L"\frac{\Delta t}{\tau}",
    xlabelsize=30,
    ylabel = L"\text{Parameter value}",
    ylabelsize=30,
    yticklabelsize  = 20,
    xticklabelsize  = 20
    )
band!(ax, helmut_data.dt,  helmut_data.A_mean - helmut_data.dA_mean, helmut_data.A_mean + helmut_data.dA_mean, color="grey70")
lines!(ax, helmut_data.dt, helmut_data.A_mean,color="black", linewidth=3, label="Amplitude estimation")
...
lines!(ax, helmut_data.dt,helmut_data.tau_mean ,color="grey70", linewidth=3, label="𝜏 estimation")
...
lines!(ax, mcmcmean_data.dt, mcmcmean_data.A,color="black", linewidth=3, label="Amplitude estimation")
lines!(ax, mcmcmean_data.dt,mcmcmean_data.tau ,color="grey40", linewidth=3, label="𝜏 estimation")
...
lines!(ax, g.ratio_deltat_tau1, g.mean_amplitude1,color="black", linewidth=3, label="Amplitude estimation")
lines!(ax, g.ratio_deltat_tau1,g.mean_tau1 ,color="grey50", linewidth=3, label="𝜏 estimation")
...
ax = Axis(f[1, 1],
    ...
    xlabelsize=30,
    ylabel = L"\text{Amplitude Estimation}",
    ylabelsize=30,
    yticklabelsize  = 20,
    xticklabelsize  = 20
    )
...
lines!(ax, a.ratio_mulitplicative_thermal[1:50], a.mean_amplitude[1:50], color="black", linewidth=3, label="Amplitude estimation")

hlines!(ax, 1.0; linestyle=:dash, linewidth=3, color = "grey30", label="True amplitude")

axislegend(ax, position = (0.8, 0.7),  framevisible = false, labelsize = 20)

text!(0.25, 0.2 ,text=L"\tau = 0.5", fontsize=50)
...
lines!(ax, d.ratio_mulitplicative_thermal[1:50], d.mean_amplitude[1:50], color="black", linewidth=3, label="Amplitude estimation")

hlines!(ax, 1.0; linestyle=:dash, linewidth=3, color = "grey30", label="True amplitude")

axislegend(ax, position = (0.8, 0.7),  framevisible = false, labelsize = 20)

text!(0.25, 0.2 ,text=L"\tau = 4.0", fontsize=50)
...
lines!(ax, e.ratio_mulitplicative_thermal[1:50], e.mean_amplitude[1:50], color="black", linewidth=3, label="Amplitude estimation")

hlines!(ax, 1.0; linestyle=:dash, linewidth=3, color = "grey30", label="True amplitude")

axislegend(ax, position = (0.8, 0.7),  framevisible = false, labelsize = 20)

text!(0.25, 0.2 ,text=L"\tau = 0.2", fontsize=50)
...
lines!(ax, fd.ratio_mulitplicative_thermal[1:50], fd.mean_amplitude[1:50], color="black", linewidth=3, label="Amplitude estimation")

hlines!(ax, 1.0; linestyle=:dash, linewidth=3, color = "grey30", label="True amplitude")

axislegend(ax, position = :rt,  framevisible = false, labelsize = 20)

text!(0.25, 0.2 ,text=L"\tau = 0.05", fontsize=50)
