...
@model oupn_thermal(rn,T,delta_t,::Type{R}=Vector{Float64}) where {R} = begin
    ...
    tau ~ Uniform(0.1,2.0)
    noise_ampl ~ truncated(Normal(0.2,0.3), lower=0)
...
@time oupndata = [sample(oupn(noisedata50[i],length(noisedata50[i]),dt[i]), NUTS(0.65), 5000) for i in eachindex(dt)]
return oupndata
